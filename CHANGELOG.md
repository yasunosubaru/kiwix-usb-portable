...
- **窗口标题显示 v1.1.2，而整包是 v1.2.0**
  - 版本号此前硬编码在 `MainWindow.xaml.cs` 的常量里，Python 版另有一份
    `APP_VER`，两者各自手动更新，跨版本后必然漂移
  - 修复：`<Version>` 写进 `KiwixWinUI.csproj` 作为唯一来源，界面在运行时读程序集
    属性；`test-gui-start.ps1` 把显示值与 csproj 逐字比对，改了 csproj 忘了其它地方
    会直接让测试失败
